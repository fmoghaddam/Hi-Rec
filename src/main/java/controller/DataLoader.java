 ...
 * @author FBM
 */
public final class DataLoader implements DataLoaderInterface{
	...
	private
	void readPersonalityFile() throws DataLoaderException {
		try (final BufferedReader reader = new BufferedReader(
				new FileReader(Globals.PERSONALITY_FILE_PATH));)
		{
			String line;
			line = reader.readLine();
			while (line.contains("#")) {
			...
			while (line != null) {
				...
				tokens = line.split(Globals.PERSONALITY_FILE_SEPERATOR);
				final FloatArrayList features = new FloatArrayList();
				final int userId = Integer.parseInt(tokens[0]);				
				for (int i = 1; i < tokens.length; i++) {
					try {
						...
						LOG.error(
								"Can not convert personality feature to numbers."
										+ exception);
						throw exception;
					}
				}
				final User user = new User(userId);
				user.setPersonalityValues(features);
				this.dataModel.addUser(user);
				line = reader.readLine();
			}
			reader.close();
			if(!alreadyReduced){
				LOG.info("Personality file loaded from "+Globals.PERSONALITY_FILE_PATH);
			}
		} catch (final Exception exception) {
			throw new DataLoaderException("Can not load personality feature file: "+ Globals.PERSONALITY_FILE_PATH, exception);
		}
	}
	...
	DataModel readData() throws DataLoaderException {
		...
		if (Globals.PERSONALITY_FILE_PATH != null && !Globals.PERSONALITY_FILE_PATH.isEmpty()) {
			hasContentUser = true;
			this.readPersonalityFile();
		}
		if (hasContentUser) {
			this.readRatingFileBaseOnExisintUsers();
		} else if (hasContent) {
	...
	private
	void readRatingFileBaseOnExisintUsers() throws DataLoaderException {
		try (final BufferedReader reader = new BufferedReader(
				new FileReader(Globals.RATING_FILE_PATH));)
		{
			String line;
