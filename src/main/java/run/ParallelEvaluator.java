...
import com.github.gaterslebenchen.libfm.core.DataMetaInfo;
import com.github.gaterslebenchen.libfm.core.FactorizationMachine;
...
public final class ParallelEvaluator {
     ...
     *
     * @param configuration Given {@link Configuration}
     */
        ...
        for (int i = 1; i <= Globals.NUMBER_OF_FOLDS; i++) {
            ...
            final Runnable task = () -> {
                try {
                    ...
                    if (algorithm instanceof FactorizationMachine) {
                        LibSVMDataProvider train = new LibSVMDataProvider();
                        LibSVMDataProvider test = new LibSVMDataProvider();

                        boolean addVisualFeatures = Globals.LOW_LEVEL_FILE_PATH.isEmpty();
                        train.convert(trainData, addVisualFeatures);
                        test.convert(testData, addVisualFeatures);

                        int num_all_attribute = Math.max(train.getFeaturenumber(),
                                test.getFeaturenumber());

                        final FactorizationMachine fm = (FactorizationMachine) algorithm;
                        {
                            ...
                            fm.init();
                            fm.hirecModel = testData;
                        }

                        FmLearnSgd fml = new FmLearnSgdElement();
                        fml.fm = fm;
                        ...
                        fml.init();
                        Arrays.fill(fml.learn_rates, fml.learn_rate);
                    ...
                    LOG.error("Fold " + foldNumber + " is done with error. Error is " + exception);
